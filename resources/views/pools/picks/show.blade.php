    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ $isSelf ?? false ? __('My Picks') : $player->name . "'s Picks" }} — {{ $pool->name }}
            </h2>
            <a href="{{ route('pools.standings', $pool) }}" class="text-sm text-gray-600 underline">{{ __('Back to standings') }}</a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white shadow-sm sm:rounded-lg p-6 text-sm text-gray-600">
                {{ __('Score:') }} <strong class="text-gray-900">{{ $membership->score }}</strong>
                · {{ __('Correct picks:') }} <strong class="text-gray-900">{{ $membership->correct_picks }}</strong>
                @if ($showFinalScore)
                    · {{ __('Final score pick:') }}
            ...
            @if ($picks->isEmpty())
                <div class="bg-white shadow-sm sm:rounded-lg p-6 text-sm text-gray-500">
                    {{ __("This player hasn't submitted picks yet.") }}
                </div>
            @elseif (empty($roundOrder))
                <div class="bg-white shadow-sm sm:rounded-lg p-6 text-sm text-gray-500">
                    {{ __("This player's picks aren't visible yet — they're revealed round by round as each round locks.") }}
                </div>
            @else
                            ...
                            @foreach ($roundMatches as $m)
                                ...
                                <li class="flex flex-wrap items-center gap-2 py-1">
                                    ...
                                    @if ($decided)
                                        ...
                                        @else
                                            <span class="text-red-500">✗ ({{ __('won:') }} {{ $m->actualWinner?->name }})</span>
                                        @endif
                                    @endif
                                </li>
                            @endforeach
