            ...
            @if ($startMatches->isNotEmpty())
                ...
                @if ($pool->status === 'setup')
                    <form method="POST" action="{{ route('pools.bracket.destroy', $pool) }}"
                          onsubmit="return confirm(@js(__('Reset the bracket? This deletes all teams and matches for this pool.')));">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-sm text-red-600 underline">{{ __('Reset bracket') }}</button>
                    </form>
                @endif
            @else
                ...
                <div class="bg-white shadow-sm sm:rounded-lg p-6"
                    ...
                    <p class="text-sm text-gray-500 mb-4">
                        {{ __('Enter both teams for each of the matchups.') }}
                        @if ($startRound !== 'FINAL')
                            {{ __('The upper bracket (onwards to Final + Third Place) is generated automatically.') }}
                        @endif
                    </p>
                    ...
                    <div class="mb-6 border rounded-md p-4 bg-gray-50">
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            {{ __('Quick fill — paste team names, one per line') }}
                        </label>
                        ...
                        <button type="button" @click="fillFromList()"
                            ...
                            {{ __('Fill matchups from list') }}
                        </button>
                        <p class="text-xs text-gray-500 mt-1">{{ __('Lines 1 & 2 become Match 1, lines 3 & 4 become Match 2, and so on.') }}</p>
                    </div>
                        ...
                        <div class="space-y-2">
                            <template x-for="(pair, index) in matchups" :key="index">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-400 w-14 text-sm" x-text="'{{ __('Match') }} ' + (index + 1)"></span>
                                    <input type="text" :name="'matchups[' + index + '][a]'" x-model="pair.a"
                                           placeholder="{{ __('Team A') }}" required
                                           class="flex-1 text-sm border-gray-300 rounded-md">
                                    <span class="text-gray-400 text-sm">{{ __('vs') }}</span>
                                    <input type="text" :name="'matchups[' + index + '][b]'" x-model="pair.b"
                                           placeholder="{{ __('Team B') }}" required
                                           class="flex-1 text-sm border-gray-300 rounded-md">
                                </div>
                            </template>
                        </div>
