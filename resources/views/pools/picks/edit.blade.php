            ...
            <div class="bg-white shadow-sm sm:rounded-lg px-6 py-4">
                <div class="grid grid-cols-3 divide-x divide-gray-100 text-center text-sm">
                    ...
                    <div class="px-4 py-2">
                        ...
                        <div class="font-semibold text-gray-800">{{ __('2️⃣ Rounds unlock as you go') }}</div>
                        <div class="text-gray-500 text-xs mt-0.5">{{ __('Quarterfinals appear after Round of 16, Semifinals after Quarterfinals, and so on.') }}</div>
                    </div>
                    <div class="px-4 py-2">
                        <div class="text-2xl mb-1">3️⃣</div>
                        <div class="font-semibold text-gray-800">{{ __('3️⃣ Save your bracket') }}</div>
                        <div class="text-gray-500 text-xs mt-0.5">{{ __('Enter the Final score, then hit Save.') }}</div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-6">
                                ...
                                <template x-for="m in matchesInRound(round)" :key="m.id">
                                    <div class="py-3">
                                        ...
                                        <template x-if="!(participants(m.id)[0] && participants(m.id)[1])">
                                            <div class="flex items-center gap-2 text-sm">
                                                <span class="text-gray-400 w-6 shrink-0 text-right" x-text="m.position + '.'"></span>
                                                <p class="text-gray-400 italic">{{ __('⬆ Pick all matches above to unlock these teams.') }}</p>
                                            </div>
                                        </template>
                                    </div>
                                </template>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <span class="text-sm text-gray-500">
                            <span x-text="completedCount()"></span> / <span x-text="matches.length"></span> {{ __('matches picked') }}
                        </span>
