<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Admin — Pool Approvals') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <h3 class="font-semibold text-gray-800 mb-3">{{ __('Pending pools') }} ({{ $pools->count() }})</h3>
                    ...
                    <div class="flex items-center justify-between py-3 border-b last:border-0">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.pools.reject', $pool) }}"
                                  onsubmit="return confirm('{{ __('Reject and delete this pool request?') }}');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                @empty
                    <p class="text-sm text-gray-500">{{ __('No pools awaiting approval.') }}</p>
                @endforelse
            </div>
        </div>
    </div>
